...
1. সর্বোচ্চ সম্ভাব্য দ্রুত গতিতে বাংলা লেখা। যেভাবে তা সম্ভব হবে: \
   ...
   - প্রয়োজনীয় সকল key, qwerty লেআউটের মধ্যেই রাখা হবে, এবং নিকটবর্তী symbols ব্যবহার করা হবে। যেমন: সেমিকোলন `;`, স্ল্যাশ `/` ইত্যাদি ব্যবহার করা হবে যেগুলো মূল qwerty লেআউটে বর্ণগুলোর সাথেই অবস্থিত।   
   - **টাচস্ক্রিনে অ্যাডাপটেশনের জন্য সাজেশন:** [নিচে উল্লেখ করা হয়েছে](#tchshkrn-ojdptshnr-jnj-shjshn)।  
2. সব ধরনের প্রয়োজনীয় এবং কম প্রয়োজনীয় বর্ণ, যুক্তবর্ণ, ও গ্লিফ মূল qwerty লেআউট থেকে হাত না সরিয়েই লেখা যাবে। যেভাবে তা সম্ভব হবে:  
   - key-sequence ব্যবহার করে প্রয়োজনীয়, কম প্রয়োজনীয় সকল কিছু লেখার ব্যবস্থা থাকবে। যেমন:   
      `ক` \= `k`, `খ` \= `kh`,   
      `ত` \= `t`, `থ` \= `th`, `ট` \= `tf`,   
      `।` (দাঁড়ি) \= `.`, `.` (বিন্দু বা ডট) \= `..`,  
      ` ়` (নুকতা) \= `,,` (দুটি কমা) 
...
2. সব ধরনের প্রয়োজনীয় ও কম প্রয়োজনীয় চিহ্ন হাতের কাছে।  
3. অ্যা ধ্বনিকে স্বর হিসেবে বিবেচনা করা হয়েছে। ae দিয়ে অ্যা লেখা যাবে এবং যেকোনো ব্যঞ্জনের পরে কারচিহ্ন হিসেবে যুক্ত করা যাবে। যেমন:\
   `র‍্যা` \= `rae`  
4. ত-বর্গীয় ও ট-বর্গীয় ব্যঞ্জনবর্ণগুলোর জন্য পৃথক কিন্তু সহজ ম্যাপিং।\
   ত-বর্গের ক্ষেত্রে:\
   `t` \= `ত`, `th` \= `থ`, `d` \= `দ`, `dh` \= `ধ`;\
   ট-বর্গের ক্ষেত্রে:\
   `tf` \= `ট`, `tff` \= `ঠ`, `df` \= `ড`, `dff` \= `ঢ`  
5. সাজেশনের প্রতি কোনো নির্ভরশীলতা নেই, কোনো সাজেশন ছাড়াই দ্রুত লেখা যাবে। তবে চাইলে সাজেশন ব্যবহার করা যেতে পারে। *(টাইপিং বুস্টারের কথা বারবার উল্লেখ করার কারণ হলো এতে পরবর্তী একাধিক শব্দ সাজেশন হিসেবে আসতে পারে, তাই সাজেশন লেখার গতি না কমিয়ে উল্টো বাড়িয়ে দেয়।)*  
6. যেখানেই সম্ভব হয়েছে নতুন-নতুন এবং সুবিধাজনক শর্টকাট তৈরি করা হয়েছে। যেমন:\
   `ক্ষ` \= `kkh` অথবা `kf`,\
   `ট্ট` \= `tftf` এর পাশাপাশি `ttf`
7. অভিনব কিছু ম্যাপিং আছে, যেমন: অ্যা, ওয়া, ওয়ে কে স্বরধ্বনির মর্যাদা দেওয়া:\
   `ae` \= `অ্যা`, `hae` \= `হ্যা`\
   `wa` \= `ওয়া`, ‌`dhwa` \= `ধোয়া`\
   `we` \= `ওয়ে`, `swetfar` \= `সোয়েটার`
# ১। ম্যাপিংয়ের সংক্ষিপ্ত সারণি

## ১.১। স্বরবর্ণ {#স্বরবর্ণ}
...
2. শব্দের শুরুতে কিংবা স্বরবর্ণের (এবং কারচিহ্নের) পরে স্বরবর্ণের পূর্ণরূপ অটো তৈরি হবে।   
   ...
   1. তবে এর ব্যতিক্রম চাইলে স্বরের আগে f যোগ করতে হবে।  
      যেমন:  
      khaif \= খাি (আ কারের পরে জোর করে ই কার যোগ করানো হয়েছে)।   
      এই আচরণের উপযোগিতা হচ্ছে ব্যঞ্জনের পরে কার চিহ্ন দিতে ভুলে গিয়ে স্পেস চেপে ফেললে; যেমন: ‘kak ’ \= ‘কাক’ লেখার পরে যদি আ-কার যোগ করতে চাই, তবে শেষের k মোছা লাগবে না। ‘kak’ এর পরের স্পেস মুছে ‘af’ লিখলেই ‘া’ যুক্ত হয়ে যাবে।  
...
3. ZWJ (zero width joiner) কোথাও কোনো কম্বিনেশনে একাধিক স্থানে যুক্ত করা সম্ভব হলে কোন দুটি বর্ণকে যুক্ত করা হবে তা নির্দিষ্ট করে দিতে ব্যবহৃত হয়। যেমন:  
   ...
   উপরে ব্যবহৃত “\~\~” হলো zwj এর জন্য ক্ষিপ্র-র ম্যাপিং। [ক্ষিপ্র-র পূর্ণাঙ্গ ম্যাপিং সেকশনে](#bshsh-kch-jktbrn-ebn-bbdh) এর উল্লেখ রয়েছে। 
...
বিভিন্ন সিস্টেমে ক্ষিপ্র কিবোর্ড ইনস্টলেশনের পদ্ধতির লিংক [ক্ষিপ্র-র ওয়েবসাইটে](https://khipro.khiproteam.pro.bd/#installation) দেওয়া আছে।
...
2. ক্ষিপ্র-র মূল রিপোজিটরি [khipro-m17n](https://github.com/rank-coder/khipro-m17n) সহ অন্যান্য কয়েকটি ল্যাংগুয়েজে ক্ষিপ্র-র যেসব মডিউল তৈরি করা হয়েছে সেগুলোর একটা তালিকা ওয়েবসাইটের হোম পেজে [ক্ষিপ্র রিপোজিটরি সমূহ](https://khipro.khiproteam.pro.bd/#repositories) সেকশনে দেওয়া হয়েছে।
